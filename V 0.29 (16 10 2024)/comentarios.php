...
else
{
	...
	if ($logado == 0)
	{
		$queryCom = mysqli_query($conexao, "SELECT u.urlFotoPerfil, u.nomeUsuario, u.idUsuario, c.dataCom, c.idComentario, c.txtComentario, u.nome,
	...
	if ($logado == 1)
	{
		...
		$queryCom = mysqli_query($conexao, "SELECT c.dataCom, c.idUsuario, c.txtComentario, c.idComentario, u.nomeUsuario, u.nome, u.urlFotoPerfil,
		COUNT(CASE WHEN cc.curtida = 1 THEN 1 END) as qtdeLikes,
		COUNT(CASE WHEN cc.curtida = 0 THEN 1 END) as qtdeDeslikes
		from comentarios c
		INNER JOIN usuario u ON c.idUsuario = u.idUsuario
		LEFT JOIN ComCurtida cc ON c.idComentario = cc.idComentario
		...
		$queryComLogado =  mysqli_query($conexao, "SELECT c.dataCom, c.idComentario, c.idUsuario, c.txtComentario, u.nome, u.nomeUsuario, c.idLit, u.urlFotoPerfil,
		COUNT(CASE WHEN cc.curtida = 1 THEN 1 END) as qtdeLikes,
		COUNT(CASE WHEN cc.curtida = 0 THEN 1 END) as qtdeDeslikes
		from comentarios c
		INNER JOIN usuario u ON c.idUsuario = u.idUsuario
				...
				<a style="text-decoration: none" href="perfil.php?i=<?php echo sha1($dadosComLogado['idUsuario']); ?>">
					...
					<div class="corpoComent">
						...
						<form method="POST" name="likeDeslikComLogado" action="BDLikeComent.php?i=<?php echo sha1($dadosComLogado['idComentario']); ?>">
							...
							<button class="botaoInvisivel" name="btnLikeComent" type="submit" style="--corSVG: var(--verde); height: 20px"><?php include('icones/GosteiFill.svg'); ?></button>
							<button class="botaoInvisivel" name="btnDeslikeComent" type="submit" style="--corSVG: var(--vermelho); height: 20px"><?php include('icones/NaoGosteiFill.svg'); ?></button>
						</form>
