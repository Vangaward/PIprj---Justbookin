...
function histLogin($dadosLit)
{
    ...
    if ($logado == 1)
    {
        ...
        // Consulta ao histórico para o usuário logado
        $querySlctHist = mysqli_query($conexao, "SELECT * FROM historico WHERE idUsuario = '$idUsuario'");
        $qtdeLinhasSelect = mysqli_num_rows($querySlctHist);

        // Inicializa variável para verificar se é necessário inserir
        $inserir = false;

        // Se houver registros no histórico, vamos verificar se o idLit já está presente
        if ($qtdeLinhasSelect > 0)
        {
            // Enquanto houver registros
            while ($itemHist = mysqli_fetch_assoc($querySlctHist))
            {
                // Verifica se o idLit já está no histórico
                if ($itemHist['idLit'] == $idLit)
                {
                    // Se encontrar o mesmo idLit, não insere novamente
                    $inserir = false;
                    break;
                }
                else
                {
                    // Se não encontrar, indica que deve inserir
                    $inserir = true;
                }
            }
        }
        ...
        {
            // Se não houver histórico, deve inserir
            $inserir = true;
        }

        // Se precisar inserir, faz a inserção no histórico
        if ($inserir)
        {
            $queryInserirtHist = mysqli_query($conexao, "INSERT INTO historico (idLit, idUsuario) VALUES ('$idLit', '$idUsuario')");
        }
    }
}
