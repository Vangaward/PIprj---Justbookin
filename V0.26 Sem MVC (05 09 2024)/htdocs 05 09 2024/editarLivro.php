<?php
//die("Acesso temporariamente negado!");
...
$_SESSION['TSFEditLit'] = bin2hex(random_bytes(32));
	...
	/*Buscar na tabela Categoria*/
		...
		if (!$queryCategoria) {echo ("Houve um erro inesperado. #2");}
	/*fim de*/

	/*Buscar categorias da literatura*/
		/*Prepared Statement - Consulta para necessidades da página*/
			$stmt = $conexao->prepare("SELECT * FROM capituloLit WHERE sha1(idLit) = ?");
			if (!$stmt) {
			...
			$stmt->close();
		/*Fim de PS*/
		/*Prepared Statement - Consulta para adicionar capítulos ao editor de capítulos*/
				$stmt = $conexao->prepare("SELECT * FROM capituloLit WHERE sha1(idLit) = ? ORDER BY numCapitulo");
				if (!$stmt) {
					die("Houve um erro inesperado. #4");
				}
				$stmt->bind_param("s", $idLiSha1);
				$stmt->execute();
				$queryCapsAddCaps = $stmt->get_result();
				if (!$queryCapsAddCaps) {
					die("Houve um erro inesperado. #4");
				}
				$stmt->close();
		...
		/*Prepared Statement - Buscar Capítulos da literatura*/
			...
			$numRowsItCat = $queryItemCat->num_rows;
			$stmt->close();
		/*Fim de PS*/
	/*fim de*/
?>
...
<div id="divConteudoId" style="display: none;">
    ...
    <div class="checkboxCat">
        <?php while ($dadosCategoria = mysqli_fetch_array($queryCategoria)) { 
            ...
            $checkboxId = sha1($idCategoria);
            $isChecked = false;
            if ($numRowsItCat > 0) {
                while ($dadosItemCat = $queryItemCat->fetch_array(MYSQLI_ASSOC)) {
                    if ($dadosItemCat['idCategoria'] == $idCategoria) {
                        $isChecked = true;
                        break;
                    }
                }
                // Reinicie o ponteiro de resultado para que ele possa ser reutilizado
                mysqli_data_seek($queryItemCat, 0);
            }
        ?>
        <input type="checkbox" name="checkboxCat[]" id="<?php echo $checkboxId; ?>" value="<?php echo $checkboxId; ?>" <?php if($isChecked) echo "checked"; ?>>
        <label for="<?php echo $checkboxId; ?>"><?php echo $dadosCategoria['nomeCategoria']; ?></label>
        <?php } ?>
    </div>
</div>
				<!---->
				...
				<label class="texto passoseditarLivro" id="labelLivro">7. Selecione a visibilidade</label>
				<br>
				<label>Pública: Qualuqer um pode acessar de qualquer lugar do site.</label>
				<br>
				...
				<?php } ?>
			<input type="button" onclick="window.location='livro.php?i=<?php echo $idLiSha1; ?>'" value="Cancelar">
			</form>
		</main>
...
function carregaCapitulosBanco(nomeCap, paginaCap) {
	...
	while ($dadosCapsAddCaps = mysqli_fetch_array($queryCapsAddCaps)) { ?>

	carregaCapitulosBanco("<?php echo $dadosCapsAddCaps['nomeCapitulo']; ?>", <?php echo $dadosCapsAddCaps['paginaInicial']; ?>);
