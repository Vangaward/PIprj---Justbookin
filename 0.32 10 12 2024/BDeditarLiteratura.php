...
if (isset($_POST['btnPublicarSemCapitulos']) || isset($_POST['btnPublicarHaCapitulos']))
{
	...
	$sqlupdateLit = "UPDATE Literatura SET titulo=?, descricao=?, status=?, idClassificacao = ? WHERE SHA1(idLit) = ?";
	$stmt = mysqli_prepare($conexao, $sqlupdateLit);
	mysqli_stmt_bind_param($stmt, "sssss", $titulo, $descricao, $status, $classificacao, $idLitSha1);
